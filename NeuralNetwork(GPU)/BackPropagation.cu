...
__global__
void cudaSoftMax_ErrorCalculation(int* categoryCount, float* results, float* softMaxResults, float* newTotalError, float* pRelu, int offset, int batchDim) {
	int d_n = threadIdx.x;
...
//Calculates Error of each Result value
__global__
void cudaResults_ErrorCalculation(float* target, float* results, float* newTotalError, float* pRelu, int offset, int batchDim) {
	int d_n = threadIdx.x;
	int BatchCount = blockIdx.x;

	newTotalError[BatchCount * batchDim + offset + d_n] = results[BatchCount * batchDim + offset + d_n] - target[BatchCount * batchDim + offset + d_n];
...
int BackPropagation::results_ErrorCalculation(dim3* kernelSizes) {
	...
	//Executes Kernel
	cudaSoftMax_ErrorCalculation << <kernelSizes[0], kernelSizes[1] >> > (resultVariables.categoryCounts, resultVariables.d_LayerResults, resultVariables.d_SoftMaxResults, resultVariables.d_TotalErrors,
